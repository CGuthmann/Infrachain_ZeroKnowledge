...
snarkjs groth16 fullprove ./input_$CIRCUIT_NAME.json "$CIRCUIT_NAME"_js/$CIRCUIT_NAME.wasm "$CIRCUIT_NAME"_0001.zkey proof_$CIRCUIT_NAME.json public_$CIRCUIT_NAME.json
START=$(date +%s.%N)
...
snarkjs groth16 verify verification_key_$CIRCUIT_NAME.json public_$CIRCUIT_NAME.json proof_$CIRCUIT_NAME.json
